...
const ListaConsultasScreen = ({ navigation, route }) => {
    ...
    getAllConsultas = async () => {
        var response = await PacientesService.getAll(`consulta`)
        setData(response)
    }
    ...
    const renderItem = ({ item }) => (
        ...
        >
            <View>
                <FontAwesome5 name={item.especialidade} style={styles.icon} />
            </View>
            <Text style={styles.name}>{item.hospital}</Text>
            <Text style={styles.doutor}>{item.doutor}</Text>
            <Text>{calcularDiferencaDeTempo(item.dataConsulta)}</Text>
